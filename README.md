...
上传区支持一次拖入或选择**多个 PDF**（单个上传依旧可用）。拖入 PDF 后**不会立即开始**：
文件会先出现在拖放区下方的**逐文件参数面板**中，为每个文件分别选好引擎 / 语言 /
并发数 / 页码范围，再点 **Start OCR** 才会真正启动任务（页码范围可把首次运行限制在
文档的一部分，只填一个边界时另一端按文档总页数补齐）。每个文件都会成为**独立任务**：
各自拥有卡片、SSE 进度流与持久化状态，可并行运行——没有单独的队列管理器。启动后
列表才从服务端 `/api/jobs` 回拉（服务端始终是任务列表的唯一数据源），运行中的任务
各自接上 SSE 进度。
